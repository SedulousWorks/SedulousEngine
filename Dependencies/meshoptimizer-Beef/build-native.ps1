...
elseif($option -eq "build")
{
    ...
    else
    {
        ...
        if($args[1] -eq "DEBUG" -or $args[1] -eq "ALL")
        {
			...
			copy-item "./build/Debug/meshoptimizer.lib" ($targetPath + "meshoptimizer.lib") -Force
			$built = $true
        }
		...
		if($args[1] -eq "RELEASE" -or $args[1] -eq "ALL")
        {
			...
			copy-item "./build/Release/meshoptimizer.lib" ($targetPath + "meshoptimizer.lib") -Force
			$built = $true
        }
