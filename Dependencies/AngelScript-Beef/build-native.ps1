...
elseif($option -eq "build")
{
    ...
    if($target -eq "RELEASE" -or $target -eq "ALL")
    {
        ...
        else
        {
            ...
            copy-item (Get-ChildItem -Recurse "./build-release" -Filter "angelscript_beef.lib" | Select-Object -First 1).FullName "$targetPath/angelscript_beef.lib" -Force
            copy-item (Get-ChildItem -Recurse "./build-release" -Filter "angelscript.lib" | Select-Object -First 1).FullName "$targetPath/angelscript.lib" -Force
        }
    }
}
