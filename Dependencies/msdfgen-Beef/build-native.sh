#!/usr/bin/env bash
# Linux counterpart of build-native.ps1. Builds the msdfgen C wrapper and msdfgen-core
# and copies both into dist/ so [Configs.*.Linux64] LibPaths can find them.
set -euo pipefail
cd "$(dirname "$0")"

SRC="msdfgen-c"
# Wrapper first: it references msdfgen-core, and a static link resolves left to right.
LIBS=("libmsdfgen-c.a" "libmsdfgen-core.a")
