...
| **Animation Graph** | `.animgraph` | DONE (core editing) | NodeGraphCanvas for states/transitions, 3D preview viewport (SkinnedMesh + AnimationGraphComponent via PreviewSceneHost), Play/Pause/Reset transport, skeleton/mesh resource pickers, parameter add/rename/delete with index remapping, transition condition editing (add/remove with param/op/threshold), clip assignment via IInspectable comptime codegen, BlendTree1D/2D entry management (add/remove with threshold/position + clip ref), blend tree parameter binding, layer management (add/switch/rename/remove), save with in-place hot-reload support. **Polish remaining:** undo/redo commands, state node type conversion, copy/paste states, drag-drop clips from asset browser, layer weight/blend mode editing. |
...
## Phase 8: VFS Mount-Correctness

The editor must address assets through the VFS (`IMount` / `IWritableMount` /
`IEnumerableMount`) rather than raw `System.IO` + absolute paths, so non-disk
writable mounts (pak-writeback, overlay, remote) work and mounts keep their
own state coherent.

**Done:**
- ✅ Write/lifecycle surface routed through `IWritableMount`: save + index
  persistence, then create / rename / delete / mkdir. `IMount.Exists` +
