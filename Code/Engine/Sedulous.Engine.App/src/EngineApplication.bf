...
abstract class EngineApplication : IDisposable, IApplicationHost
{
	...
	// Distinct from mBuiltinAssetDirectory which is the engine's discovered Assets root
	...
	// The mount exposes raw bytes in mBuiltInAssetDirectory; the index, if a
	...
	/// otherwise. Distinct from BuiltInAssetDirectory (engine-shipped Assets).
