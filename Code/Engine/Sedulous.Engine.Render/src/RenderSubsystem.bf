...
class RenderSubsystem : Subsystem, ISceneAware, ISceneRenderer, IScreenRenderer
{
	...
	// Probe system (owns ProbePipeline, ProbeResources, convolution)
	private ProbeSystem mProbeSystem ~ delete _;
	...
	public String BuiltInAssetDirectory { get; set; }
	...
	protected override void OnInit()
	{
		...
		// Probe system (capture pipeline + resource management + convolution)
		mProbeSystem = new ProbeSystem();
		mProbeSystem.Initialize(mRenderContext);
	...
	/// Delegates probe capture to ProbeSystem (Renderer layer).
	private void CaptureProbes(ICommandEncoder encoder, RenderView mainView, Pipeline pipeline, int32 frameIndex)
	{
		mProbeSystem.CaptureProbes(encoder, mainView, pipeline, frameIndex);
	}
	...
	public void OnSceneDestroyed(Scene scene)
	{
		...
		// Destroy all probe resources (re-created if scene is reopened)
		mProbeSystem.DestroyAllProbes();
