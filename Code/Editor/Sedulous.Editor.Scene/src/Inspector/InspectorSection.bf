...
class InspectorSection
{
	...
	/// Per slot rows for a list of reflected objects: after the list editor, one section of
	/// the ELEMENT's own rows per slot, titled by its name when it has one.
	///
	/// A slot's rows are the element type's, generated by the same emitter as a component's,
	/// so a slot field reads, writes, picks and undoes exactly like a component field. The
	/// slot's target resolves the element from the owning instance on every access, since the
	/// component pool and the list both move.
	public void SlotSections<TElem>(StringView field, delegate List<TElem>(void* p) read,
		delegate void(TElem element, String outTitle) title) where TElem : class
	{
		let container = Own(field);
		Keep(read);
		Keep(title);
		...
		{
			let heading = new String();
			Keep(heading);
			title(list[i], heading);

			delegate void*(void* owner, int slot) resolve = new [=read](owner, slot) =>
			{
				let live = read(owner);
				return (slot < live.Count) ? Internal.UnsafeCastToPtr(live[slot]) : null;
			};
			let slotTarget = new SlotTarget(mTarget, typeof(TElem), container, i, resolve);
			Keep(slotTarget);

			let section = new InspectorSection(mOwner, slotTarget, heading);
			Keep(section);
			InspectorRows<TElem>.Build(section);
		}
	}
