...
/// Adds the sculpt, splat and hole brushes to every viewport tool set that has a scene and a
/// command stack.
class TerrainViewportToolProvider : IViewportToolProvider
{
	public void CreateTools(ViewportToolManager manager, in ViewportToolHostContext context)
	{
		...
		manager.Add(new TerrainHoleTool(context.Scene, context.Commands, context.AssetEdits));
	}
}
