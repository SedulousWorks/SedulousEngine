...
/// The bus tree's adapter: label rows, never reorderable. The snapshot, the tree and the
/// asset are BORROWED.
class BusLayoutTreeAdapter : IReorderableTreeAdapter
{
	private BusTreeSnapshot mSnapshot;
	private TreeView mTree = null;
	private AudioBusLayoutAsset mAsset = null;

	public this(BusTreeSnapshot snapshot)
	{
		mSnapshot = snapshot;
	}

	public void SetTree(TreeView tree) => mTree = tree;
	public void SetAsset(AudioBusLayoutAsset asset) => mAsset = asset;
	...
	public void BindView(View view, int32 nodeId, int32 depth, bool isExpanded)
	{
		...
		if (let label = row.GetChildAt(0) as Label)
			label.SetText(mSnapshot.Label(mAsset, nodeId));
		if (mTree != null)
