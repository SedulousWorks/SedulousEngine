...
public class FlattenedTreeAdapter : IListAdapter, ITreeAdapterObserver
{
	...
	/// Get all expanded node IDs (copies to output set).
	public void GetExpandedNodes(HashSet<int32> output)
	{
		for (let nodeId in mExpanded)
			output.Add(nodeId);
	}

	/// Bulk-set expanded nodes and rebuild. Only expands nodes that actually have children.
