...
class ParticleEffectResourceManager : ResourceManager<ParticleEffectResource>
{
	...
	protected override Result<void, ResourceLoadError> ReloadResource(ParticleEffectResource resource, ResourceLoadContext ctx)
	{
		if (SerializerProvider == null)
			return .Err(.NotSupported);
		...
		// Dispatch through Reload so the existing ParticleEffect instance
		// is reused (ClearForReload empties it in place). Reading via
		// Serialize directly would delete outside references.
