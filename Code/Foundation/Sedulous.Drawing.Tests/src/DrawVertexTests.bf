...
class DrawVertexTests
{
	...
	{
		let vertex = DrawVertex(10.0f, 20.0f, 0.5f, 0.75f, Color32.Red);
		...
		Test.Assert(vertex.Color == Color32.Red);
	}
	...
	public static void Constructor_Vector2_SetsPositionAndTexCoord()
	{
		...
		let vertex = DrawVertex(pos, uv, Color32.Green);

		Test.Assert(vertex.Position == pos);
		Test.Assert(vertex.TexCoord == uv);
		Test.Assert(vertex.Color == Color32.Green);
	}
	...
	public static void Default_AllZero()
	{
		...
		Test.Assert(vertex.Color == Color32(0, 0, 0, 0));
	}
}
