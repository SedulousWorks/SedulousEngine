...
///
/// DELIBERATELY DUPLICATED with the toolkit's toast host, which is the same behaviour under a
/// different namespace. The game kit builds on the UI proper and never on the toolkit, and the
/// toolkit never reaches into the game kit, so neither can be the other's dependency.
