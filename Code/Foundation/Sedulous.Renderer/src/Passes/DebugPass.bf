...
class DebugPass : PipelinePass
{
	...
	public override void AddPasses(Sedulous.RenderGraph.RenderGraph graph, RenderView view, Pipeline pipeline)
	{
		...
		if (debugDraw == null || debugDraw.LineVertexCount == 0)
			return;
	...
	private void Execute(IRenderPassEncoder encoder, RenderView view, Pipeline pipeline, bool hasDepth)
	{
		...
		// Upload line vertices to the per-frame GPU buffer.
		let vertCount = debugDraw.LineVertexCount;
		if (vertCount == 0) return;
		let clamped = Math.Min((uint32)vertCount, DebugDrawSystem.MaxLineVertices);

		let vb = debugSystem.GetLineVertexBuffer(view.FrameIndex);
		TransferHelper.WriteMappedBuffer(vb, 0,
			Span<uint8>((uint8*)debugDraw.LineVertices.Ptr, (int)(clamped * DebugVertex.SizeInBytes)));
		...
		// Material + draw-call bind groups still required by the pipeline layout;
		// bind defaults so the descriptor sets are valid even though debug shaders
		// don't sample them.
