...
public class NodeGraphCanvas : View
{
	...
	public override void OnDraw(UIDrawContext ctx)
	{
		...
		if (mInteraction == .BoxSelecting)
		{
			...
			ctx.VG.FillRect(rect, Color(accentColor.R, accentColor.G, accentColor.B, 40 / 255.0f));
			ctx.VG.StrokeRect(rect, Color(accentColor.R, accentColor.G, accentColor.B, 150 / 255.0f), 1);
		}
	...
	private void DrawGrid(UIDrawContext ctx)
	{
		...
		let gridColor = Color(borderColor.R * 0.7f, borderColor.G * 0.7f, borderColor.B * 0.7f, borderColor.A);
	...
	private void DrawConnection(UIDrawContext ctx, NodeGraphConnection conn, bool hovered)
	{
		...
		if (conn.IsSelected)
			...
			color = Color(Math.Min(1.0f, color.R + 40 / 255.0f), Math.Min(1.0f, color.G + 40 / 255.0f), Math.Min(1.0f, color.B + 40 / 255.0f), 230 / 255.0f);
