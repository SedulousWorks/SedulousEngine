...
public class SplitView : ViewGroup
{
	...
	protected override void OnLayout(float left, float top, float width, float height)
	{
		...
		if (Orientation == .Horizontal)
		{
			...
			if (mFirst != null)
			{
				mFirst.Measure(BoxConstraints.Tight(firstW, h));
				mFirst.Layout(0, 0, firstW, h);
			}
			if (mSecond != null)
			{
				let secondX = firstW + divSize;
				mSecond.Measure(BoxConstraints.Tight(secondW, h));
		...
		else
		{
			...
			if (mFirst != null)
			{
				mFirst.Measure(BoxConstraints.Tight(w, firstH));
				mFirst.Layout(0, 0, w, firstH);
			}
			if (mSecond != null)
			{
				let secondY = firstH + divSize;
				mSecond.Measure(BoxConstraints.Tight(w, secondH));
	...
	public override void OnDraw(UIDrawContext ctx)
	{
		...
		let divColor = mDividerHovered || mDragging
			? ResolveStyleColor(.AccentColor, .(80, 85, 105, 255))
			: ResolveStyleColor(.BorderColor, .(55, 58, 70, 255));
		ctx.VG.FillRect(divRect, divColor);

		// Grip indicator in divider center.
		let gripColor = ResolveStyleColor(.TextDimColor, .(100, 105, 120, 180));
