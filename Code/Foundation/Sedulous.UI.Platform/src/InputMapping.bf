...
/// Utility class for mapping Platform input types to UI2 input types.
/// Includes US-layout text input emulation for when SDL_StartTextInput
/// is not active.
static class InputMapping
{
	/// Maps Platform.Input.KeyCode to UI2.KeyCode.
	/// Values match by design - trivial cast.
	public static Sedulous.UI.KeyCode MapKey(Sedulous.Platform.Input.KeyCode platformKey)
	{
		return (Sedulous.UI.KeyCode)(int)platformKey;
	}

	/// Maps Platform.Input.KeyModifiers to UI2.KeyModifiers.
	/// Collapses Left/Right variants into combined flags.
	public static Sedulous.UI.KeyModifiers MapModifiers(Sedulous.Platform.Input.KeyModifiers platformMods)
	{
		Sedulous.UI.KeyModifiers result = .None;
		if (platformMods.HasFlag(.LeftShift) || platformMods.HasFlag(.RightShift))
			result |= .Shift;
		if (platformMods.HasFlag(.LeftCtrl) || platformMods.HasFlag(.RightCtrl))
			result |= .Ctrl;
		if (platformMods.HasFlag(.LeftAlt) || platformMods.HasFlag(.RightAlt))
			result |= .Alt;
		if (platformMods.HasFlag(.CapsLock))
			result |= .CapsLock;
		if (platformMods.HasFlag(.NumLock))
			result |= .NumLock;
		return result;
	}

	/// Maps Platform.Input.MouseButton to UI2.MouseButton.
	public static Sedulous.UI.MouseButton MapMouseButton(Sedulous.Platform.Input.MouseButton platformButton)
	{
		return (.)platformButton;
	}

	/// Maps a UI-layer cursor request to the platform's CursorType.
	...
	public static Sedulous.Platform.Input.CursorType MapCursor(Sedulous.UI.CursorType uiCursor)
	{
		switch (uiCursor)
		{
	...
	/// Converts a platform key code to a printable character (US keyboard layout).
	/// Returns '\0' if the key doesn't produce a printable character.
	/// Fallback for when SDL_StartTextInput is not active.
	public static char32 KeyToChar(Sedulous.Platform.Input.KeyCode key, bool shift)
	{
		// Letters A-Z
		if (key >= .A && key <= .Z)
		{
