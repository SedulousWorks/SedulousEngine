...
public class ToggleSwitch : View
{
	...
	public override void OnDraw(UIDrawContext ctx)
	{
		...
		// Track.
		let trackBounds = RectangleF(0, trackY, TrackWidth, TrackHeight);
		if (!ctx.TryDrawDrawable("ToggleSwitch.Track", trackBounds, GetControlState()))
		{
			...
			ctx.VG.FillRoundedRect(trackBounds, trackRadius, trackColor);

			// Track border.
			let borderColor = ctx.Theme?.GetColor("ToggleSwitch.Border", .(80, 85, 100, 255)) ?? .(80, 85, 100, 255);
			ctx.VG.StrokeRoundedRect(trackBounds, trackRadius, borderColor, 1);
		}
		...
		// Knob.
		let knobBounds = RectangleF(knobX, knobY, KnobSize, KnobSize);
		if (!ctx.TryDrawDrawable("ToggleSwitch.Knob", knobBounds, GetControlState()))
		{
			// Knob shadow.
