...
/// equivalent, so the directory is WALKED UP from the working directory instead: the sandbox
/// runs from the workspace root, from its own project directory, or from the build output, and
/// the data sits at the repository root in all three cases.
static class SandboxContent
{
	...
	/// data. False rather than an assertion, because the sandbox still shows everything that
	/// does not need it.
