...
static class PipelineScriptSurfaceTests
{
	...
	public static void TheCountIsATripwire()
	{
		...
		// Bump deliberately when a type is marked or unmarked: the runtime's 156, the two
		// Pipeline domain enums, and three foundation enums only the pipeline links.
		Test.Assert(PipelineScriptSurface.TypeCount == 161, scope $"the pipeline surface has {PipelineScriptSurface.TypeCount} types");
	}
